...
/* Task 2: There is a typo in your dataset 😱 The 9th artist, Vincent Van Gogh is currently Vincent Van Dough. Use an array method to fix this 
...
function get20s(array, birthYear, deathYear){
  ...
  let theOnlyOnes = [];
    for (let i = 0; i < array.length; i++){
      if(array[i].years >= birthYear && array[i].years <= deathYear){
        theOnlyOnes.push(array[i].name);
    }
  }
  return theOnlyOnes;
}
...
<div class = "bio">Vincent Willem van Gogh (Dutch: [ˈvɪnsɛnt ˈʋɪləm vɑŋ ˈɣɔx] (listen); 30 March 1853 – 29 July 1890) was a Dutch 
Post-Impressionist painter who is among the most famous and influential figures in the history of Western art. In just over a decade he created 
about 2,100 artworks, including around 860 oil paintings, most of them in the last two years of his life. They include landscapes, still lifes, 
portraits and self-portraits, and are characterised by bold colours and dramatic, impulsive and expressive brushwork that contributed to the 
foundations of modern art. However, he was not commercially successful, and his suicide at 37 followed years of mental illness and poverty.</div>
</div>

Create a function called `getHTML()` that takes the parameter `data` and uses a for..in loop to format and console.log data like the example 
above. 
...
function randomize(/* Code here */){
 ...
 /* STRETCH 3: Use advanced array methods (.map, .reduce, .filter) to refactor your MVP code (create an array of all artists born in the 1900s 
